...
- **Responsive/mobile:** every page's styles must work well on mobile screens too, not just desktop — grids of cards (concert items, artist folders, images) should collapse to fewer columns or a single column on narrow viewports, the navbar should adapt to small screens: the sub-page tabs collapse into a hamburger menu, while the "Login with Spotify" button and the Settings gear icon stay visible on mobile as compact icon buttons rather than being hidden in the menu, and touch targets (buttons, tabs) should be large enough to tap comfortably. Use relative units and CSS media queries throughout rather than fixed desktop-only widths — this applies to every page in the app, not just specific ones.
- **Icons:** navbar icons (hamburger, login, settings gear) and any other UI icons are inline SVGs, hand-picked from a free set (e.g. Feather or Heroicons) and pasted directly into the markup/CSS — no icon font or external icon library dependency.
...
6. Build the navbar partial (rendered into every page), containing:
   ...
   - Sub-page tabs, linking to (the root `/` redirects to `/concert-date-fetcher`, so that tab is open by default):
     ...
     | Artist Graph | `/followed-artists-graph` | internal page, content defined later |
...
5. Flatten the chosen setlist's sets (including encores) into one ordered list of songs in their original order, and render them with a remove ("✕") control on each. Entries marked by setlist.fm as played from tape (intros/outros rather than a song performed live) stay in the list, marked with a "Tape" label next to the title. Entries setlist.fm marks as a cover get a "Cover" label next to the title (both labels styled like Concert Date Fetcher's "Support" pill), using the original artist's name from that data for the matching step below instead of the artist that was searched for.
...
- On click: `POST` the resolved, ordered list of track URIs (in the same order as the — possibly trimmed — setlist) to `/playlists/{playlist_id}/items` (Spotify retired the older `/tracks` endpoint in its February 2026 API changes), appending them to the end of the selected playlist.
...
7. Append this run's new releases to the list and save it to `localStorage` — the previously shown releases stay, and the new finds join them, the whole list sorted by release date (most recent first). If the list is still empty, show "No new releases found since your last check." instead of an empty area. A failed artist lookup needs no special handling: that artist's releases just aren't marked as seen, so the next check that gets through to them still shows them. A cancelled check still appends and stores the new releases it found up to that point, handled exactly like a finished check — the artists it never got to aren't marked as seen, so the next check covers them. If it hadn't found anything new yet, the previous list stays as it was. After every check, finished or cancelled, a short note under the progress bar says what it found — worded and fading out exactly like Concert Date Fetcher's ("Found 12 new releases for 8 artists." / "No new releases found." / "Check cancelled — found 3 new releases for 2 artists."): shown for 6 seconds, then faded out over 0.6 seconds, and hidden straight away when the next check starts.
...
- Grid: four big cards per row on desktop, so there's room to see each record come out, three on tablets (≤ 900px) and two on phones (≤ 600px).
- Card styling: rounded corners with a plain grayish border, and the concert cards' lift on hover — but instead of their purple glow and foil sheen, **the record itself slides out of its sleeve**: the cover art slides left, and the vinyl, drawn in CSS to match its Discogs format, glides out to the right and spins (at 33⅓, 45 or 78 RPM, per the format), while the card glows in the record's own colours. `public/js/vinyl.js` reads the format text:
  - **Colours** — words and phrases ("Oxblood", "Baby Blue", "Coke Bottle Clear", …) map to `--vinyl-*` tokens in `variables.css` (real-world pressing colours; never hex values anywhere else). Phrases like "Black Friday" or "White Label" don't count; a bracketed marketing name ("[Portofino Orange Glitter]") is only read when the plain description names no colour. No colour at all means classic black. "Rainbow" (also "Multicolour", "Spectrum", "Prism") means the whole spectrum — red, orange, yellow, green, blue, purple — in place of named colours: a rainbow swirl gets one twisted arm per colour filling the disc, plain "Rainbow" a smooth colour wheel, and rainbow splatter / stripes / marble / smash use the spectrum as their colours (after the base colour when one is named, e.g. "White With Rainbow Splatter").
  - **Pattern** — marble (stripes bent into veins by an SVG turbulence filter), swirl / galaxy (twisted spiral arms, galaxy with stars), zoetrope (rings of 16 frames printed on the record, black unless another colour is named: a ball bouncing outwards and a propeller turning. Instead of spinning smoothly, the disc turns in steps of exactly one frame slot, the way a strobe light shows a real zoetrope, so every figure animates in place while the record seems to stand still. It turns once every 0.9s whatever the rpm - the turn doesn't show, and it makes about 18 frames a second, smooth rather than jerky; on the Collection's turntable it keeps stepping the same way. The frame count and turn time, `ZOETROPE_FRAMES` and `ZOETROPE_TURN_MS` in `vinyl.js`, have to match the `steps(16)` and `0.9s` on `.vinyl--zoetrope .vinyl__disc` in `style.css`), colour smash / blob / merge (big blobs of the colours pressed together and torn into irregular, crisp-edged patches by strong turbulence; while the record spins the turbulence swells and eases, so the patches squish and flow like molten vinyl in the press), peppermint / candy cane (candy stripes, red and white unless another colour is named, that twist tighter and relax again while the record spins — an SVG shape animation; like the smash's, it's paused whenever the record isn't out and never runs with reduced motion), splatter, half & half, tri-colour / stripes, colour-in-colour ("Yellow In Clear"), smoke (fractal-noise wisps) and picture disc (the cover art as the record). Randomised per release id, so each card's marbling or splatter is its own but the same on every visit.
  - **Finish** — metallic (brushed-metal bands that shimmer as it spins), glitter (in its named colour: "With Gold Glitter"), translucent / clear (see-through with light coming through), glow in the dark (it glows).
  - Every record has grooves, the cover art as its centre label and a spindle hole; a still reflection sits on top, with a highlight that follows the cursor.
  - **The card's text area takes on the record's pattern too**: as the record comes out, the same drawing (same seed, so the same marbling or swirl) fades in behind the title, artist and details, under a dark wash that keeps the text readable on even the lightest records. On touch screens it's always shown, like the peeking record.
  - The record is only drawn on a card's first hover (or keyboard focus), so hundreds of cards stay light. Touch screens can't hover, so there every card shows a sliver of its record peeking out from behind the sleeve, drawn as the card scrolls into view. With reduced motion, nothing slides or spins. There's no new-vs-old distinction here (unlike Concert Date Fetcher and Followed Artists Graph) — since every render only ever contains the current check's fresh finds, there's nothing "old" left on screen to distinguish from.
...
Routes: `/store` and `/collection`, both tabs after Albums (the Album Suggester's tab, labelled "Albums"), behind the usual Spotify-login gating.

**Coins:** earned on the Album Suggester — every revealed suggestion (first open and every re-roll) pays its rarity tier: blue 50, purple 100, pink 250, red 500, gold 1,000 (`TIER_COINS` in `album-suggester.js`). Marking that album listened pays the same amount again, but only when it actually joins the album history. The balance sits in the Store tab next to its label (on phones, where the tabs fold into the hamburger menu, as a coin chip beside the profile pill), with the coin icon: gold, a quaver in the middle (the `#coin-icon` symbol in `navbar.ejs`). However coins are earned, `MusicHub.wallet.earn(amount, { from })` stores them straight away and plays the same effect: a "+N" label and coins bursting out of `from`, flying up to the balance and counting it up as each lands with a synthesised clink. More coins, more flying, each rarity clearly more than the one below: 10 for 50, 16 for 100, 31 for 250, 50 for 500, 81 for 1,000 (at most 90). They all set off within 1.8 s, so a big shower comes thicker, not longer, and at most 30 of them clink. Spending counts the balance down.

**Username styles:** Rainbow (1,000) — the spectrum sliding through the name and the pill's border alike, in step — and Gold Shine (2,000) — a turning gold border and a shine sweeping across the name — on the navbar's profile pill. An item not bought yet shows a lock over its preview; without enough coins the card is greyed out and its Buy button disabled, from the first paint. Buying plays the unlock (the lock strains, the shackle springs open, the lock lifts away, a shine sweeps across the item), then the item can be equipped / unequipped. The equipped style is set on `<html data-name-style>` by the head partial before the first paint, so it never flashes.

**Vinyl:** one Mystery Vinyl offer, 1,000 coins. Unbox first opens an album picker - the Album Suggester's album history, newest first, searchable by album or artist (an empty history points to the Albums page instead) - and the new vinyl is pressed for the album chosen: its cover becomes the record's centre label and, on the Collection, its sleeve. Then the unboxing drops a mailer box in, which keeps hopping and rattling ("Click to open") until it's clicked; then the lid slides off, the record peeks out over the rim and ducks back in, and waits there with "Drag the vinyl out" underneath. The user drags it up and out (Enter, Space or Arrow Up does it too): it grows to full size as it rises, the box falls away and a burst of light comes up behind it, over the whole dialog, text included. Let go past halfway and it comes the rest of the way; short of that it slides back into the box - with its name, the album it was pressed for, "Added to collection", "View collection" and "Close". The coins are only taken, and the record only picked and added to the collection, when the box is opened; Escape before that closes the dialog at no cost. Once the record is out, a click on the backdrop closes the dialog too (not before). The dialog keeps one size throughout: the hint and the result share one slot, sized for the result. The record is a Discogs-style format ("Oxblood & Cream Marble") drawn by `vinyl.js`, from a catalog of 2,813 in `vinyl-catalog.js`: a family is rolled by weight first (zoetrope and glow in the dark rarest), then a record from it that the collection doesn't hold — never a duplicate. Picture discs (the album's cover printed across the whole record - plain, with glitter, metallic, clear or glow in the dark) are only rolled for an album with cover art. The Collection page ("Your vinyl collection", with "12 of 2,813 collected" beside it) lists them newest first on the Discogs page's release cards: the album's cover as the sleeve, the record sliding out and spinning on hover (peeking out on touch screens), the pattern behind the card's text, the record's style as a tag in the cover's top-left corner (the Concert cards' "New" tag; cut short with its full text in a tooltip if too long), and below it the album, the artist, "Added <date>" and a **Play** button. A vinyl unboxed before albums were picked has the app's logo for a sleeve and label, and no Play. Between the heading and the cards, once there's a vinyl, a toolbar like the album history's: a search field on the left (style, album and artist; ignoring case and accents, every word has to match; "No vinyls match "…"." when nothing does; not stored) and a Date sort switch on the right, newest first by default, flipped by clicking it. On phones the search takes its own full-width row.
...
**Persistence:** one `store` key (in `APP_DATA_KEYS`, so it travels with Export/Import). Settings' "Clear all data" never removes it - coins, bought and equipped items and the vinyl collection all stay (`KEPT_ON_CLEAR` in `storage.js`) - and its confirm dialog says so.

```
{
  credits: 1550,
  owned: ["username:rainbow"],
  equipped: { username: "rainbow" },
  vinyls: [{
    format: "Oxblood & Cream Marble", family: "Marble", seed: "k3j9x2pq", unboxedAt: "2026-09-25T10:00:00Z",
    // The album it was pressed for (missing on vinyls unboxed before albums were picked).
    album: { id: "<spotify album id>", name: "...", artist: "...", imageUrl: "...", spotifyUrl: "..." }
  }]
}
```
